...
import { reservationsCsv, customersCsv, entriesCsv, type CsvFile } from '@lib/csv';

export const prerender = false;

// GET /api/admin/export?type=reservations|customers|entries[&from=&to=]
//
...
export const GET: APIRoute = async ({ url, locals }) => {
  ...
  try {
    ...
    if (type === 'customers') return csvResponse(await customersCsv(supabase));
    if (type === 'entries') return csvResponse(await entriesCsv(supabase));
