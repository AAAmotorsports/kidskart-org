...
`TESTING.md` に、予約 → 変更 → キャンセル → 電話予約 → 営業状況・路面状況 → メールまでを
...
貸切 / ブロック / 営業状況と路面状況 / 変更の再判定 / キャンセルと無断キャンセル / 強制受付 /
...
| URL | 内容 |
|---|---|
| `/` | **今日走れる？** カテゴリー別 ○△✕— と RP の空き、営業状況・路面状況 |
...
| `/reserve/event` | **イベント参加申込** (耐久 / スプリント / シリーズ戦) |
| `/r/[token]` | 予約者専用ページ (確認 / 日時・人数変更 / キャンセル、ログイン不要) |

### 管理向け (ログインが必要)

`/admin/login` でユーザー名とパスワードを入れます。ログインは 30 日ぶん
保たれるので、ふだんは開くだけで使えます (ブラウザのパスワード保存も効きます)。

| URL | 内容 |
|---|---|
| `/admin/login` | ログイン |
| `/admin` | 今日のダッシュボード・**営業状況/路面状況のワンタップ**・要対応一覧・今週 |
| `/admin/sheet` | 本日の受付シート (スマホ / A4 印刷。Web・電話・店頭の全予約) |
| `/admin/calendar` | 月 / 週カレンダー + 予定のまとめて登録 |
| `/admin/day/[date]` | 日別運営画面 (営業状況/路面状況・一括連絡/個別連絡・予約操作・代理入力・強制受付・予定登録) |
| `/admin/reserve` | **予約の登録** (電話・店頭。トップの「📞 予約を登録」から) |
| `/admin/reservations` | 予約台帳の検索 |
| `/admin/customers` | 顧客一覧・履歴・スタッフメモ |
| `/admin/entries` | **イベント参加申込の一覧** (確定・入金記録・CSV) |
...
| 随時 | 営業状況の変更・お知らせ (一括/個別) | `/admin/day/[date]` |
