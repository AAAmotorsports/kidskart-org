...
// Response: { ok: true, reservation_number, cancelled_at }
...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  const parts = (res.reservation_participants as any[])?.map((p) => p.name_snapshot) ?? [];
